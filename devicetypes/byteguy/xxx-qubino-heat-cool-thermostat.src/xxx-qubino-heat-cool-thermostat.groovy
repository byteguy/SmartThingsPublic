 ...
 *
 *
 * |---------------------------- DEVICE HANDLER FOR QUBINO FLUSH Heat Cool THERMOSTAT Z-WAVE DEVICE -------------------------------------------------------|  
 *	The handler supports all unsecure functions of the Qubino Flush Heat Cool Thermostat device, except configurable inputs. Configuration parameters and
 ...
 *   Hacked by Byteguy to work with Qubino Heat Cool Termostat, and only that.
 */
metadata {
	definition (name: "XXX Qubino Heat Cool Thermostat", namespace: "byteguy", author: "Byteguy, Kristjan Jam&scaron;ek") {
        ...
        fingerprint mfr:"0159", prod:"0005", model:"0052", deviceJoinName: "Qubino Heat Cool Thermostat"  //Manufacturer Information value for Qubino Flush Heat Cool Thermostat
	}
	...
	tiles(scale: 2) {
		multiAttributeTile(name:"thermostat", type:"thermostat", width:6, height:4) {
			...
			tileAttribute("device.thermostatOperatingState", key: "OPERATING_STATE") {
				...
				attributeState("auto", backgroundColor:"#e86d13")  // was heating
//				attributeState("cooling", backgroundColor:"#00A0DC")
			}
			tileAttribute("device.thermostatMode", key: "THERMOSTAT_MODE") {
				attributeState("Off", label:'${name}')
				attributeState("Auto", label:'${name}')   // Was Heat
//				attributeState("Cool", label:'${name}')
			}
			tileAttribute("device.autoSetpoint", key: "AUTO_SETPOINT") {
				attributeState("autoSetpoint", label:'${currentValue}', unit:"dF", defaultState: true)
			}
...
*/
				...
				input name: "param43", type: "number", range: "0..1255", required: false,
					title: "43. Hysteresis Heating On.\n" +
				...
				input name: "param44", type: "number", range: "0..1255", required: false,
					title: "44. Hysteresis Heating Off.\n" +
						   "This parameter defines temperature min difference between real measured temperature and set-point temperature to turn device on.\n" +						   
						   "Available settings:\n" +
							"0 - 127 = 0.0 °C ... 12.7°C.\n" +
							"1001 - 1127 = - 0.1°C ... - 12.7 °C\n" +
							"Default value: 5 (+ 0.5 °C)" 
				input name: "param45", type: "number", range: "0..1255", required: false,
					title: "45. Hysteresis Cooling On.\n" +
						   "This parameter defines temperature min difference between real measured temperature and set-point temperature to turn device on.\n" +						   
						   "Available settings:\n" +
							"0 - 127 = 0.0 °C ... 12.7°C.\n" +
							"1001 - 1127 = - 0.1°C ... - 12.7 °C\n" +
							"Default value: 5 ( 0.5 °C)." 			   
				input name: "param46", type: "number", range: "0..1255", required: false,
					title: "46. Hysteresis Cooling Off.\n" +
						   "This parameter defines temperature min difference between real measured temperature and set-point temperature to turn device on.\n" +						   
						   "Available settings:\n" +
							"0 - 127 = 0.0 °C ... 12.7°C.\n" +
							"1001 - 1127 = - 0.1°C ... - 12.7 °C\n" +
							"Default value: 1002 (+ 0.2 °C)"                             
				input name: "param47", type: "number", range: "0..1127", required: false,
					title: "47. Antifreeze.\n" +
				...
				input name: "param64", type: "enum", required: false,
					...
					title: "64. Output Switch selection Q1.\n" +
						   ...
						   "Available settings:\n" +
							...
							"Default value: 0."
				input name: "param65", type: "enum", required: false,
					options: ["0" : "0 - When system is turned off the output is 0V (NC)",
							  "1" : "1 - When system is turned off the output is 230V or 24V (NO)"],
					title: "65. Output Switch selection Q2.\n" +
...
def modeSetting() {
	...
	def cmds = []
    log.debug "I'm in modeSetting: $currentMode"
	switch(currentMode){
		case "Off":
...
//			}
		...
		case "Auto":  // Was Heat
			cmds << zwave.thermostatModeV2.thermostatModeSet(mode: 3).format()
			cmds << zwave.thermostatSetpointV2.thermostatSetpointGet(setpointType: 1).format()     // was 1
			cmds << zwave.thermostatModeV2.thermostatModeGet().format()
			break;
//		case "Cool":
...
*/
def tempUp() {
	def spIncrease = roundToHalf(device.currentValue("thermostatSetpoint")) + 1
...
*/
def tempDown() {
	def spDecrease = roundToHalf(device.currentValue("thermostatSetpoint")) - 1
...
def setSpValue(setpoint){
	...
	def cmds = []
    def paul = []
    log.debug "In setSpValue $setpoint"
    log.debug "In setSpValue currentMode = $currentMode"
    log.debug "cmds object insoect $cmds.inspect"
switch(currentMode){
		case "Off":
			log.debug "In Off mode, setting setpoint to new value setpoint"
...
////					cmds << zwave.thermostatSetpointV2.thermostatSetpointSet(precision: 1, scale: 0, scaledValue: setpoint, setpointType: 3, size: 2).format()
////				}else{
////					cmds << zwave.thermostatSetpointV2.thermostatSetpointSet(precision: 1, scale: 1, scaledValue: setpoint, setpointType: 3, size: 2).format()
////				}
...
//			}
          ...
          break;
		case "Auto":
			log.debug "In Auto mode, setting setpoint to new value"
////			sendEvent(name: "autoSetpoint", value: setpoint, displayed: false)
////			sendEvent(name: "thermostatSetpoint", value: setpoint, displayed: false)
////			if(location.temperatureScale == "C"){
				cmds << zwave.thermostatSetpointV2.thermostatSetpointSet(precision: 1, scale: 0, scaledValue: setpoint, setpointType: 1, size: 2).format()
...
//			break;
        ...
        //log.debug "Object cmds $cmds.status"
        //cmds << zwave.thermostatSetpointV2.thermostatSetpointSet(precision: 1, scale: 0, scaledValue: setpoint, setpointType: 1, size: 2).format()
...
*/
def configure() {
	log.debug "Qubino Flush Heat Cool Thermostat:configure()"
	state.numEndpoints = 0
	state.coolModeEnabled = false
	//log.debug state.numEndpoints
	def assocCmds = []
	assocCmds << zwave.thermostatSetpointV2.thermostatSetpointGet(setpointType: 3).format()
...
def refreshPowerConsumption() {
	log.debug "Qubino Flush Heat Cool Thermostat:refreshPowerConsumption()"
	delayBetween([
...
def setAssociation() {
	log.debug "Qubino Flush Heat Cool Thermostat:setAssociation()"
	def assocSet = []
...
def setConfiguration() {
	...
	if(settings.param46 != null){
		configSequence << zwave.configurationV1.configurationSet(parameterNumber: 46, size: 2, scaledConfigurationValue: settings.param46.toInteger()).format()
	}    
	if(settings.param47 != null){
		configSequence << zwave.configurationV1.configurationSet(parameterNumber: 47, size: 2, scaledConfigurationValue: settings.param47.toInteger()).format()
	}
...
//	}
	...
	if(settings.param64 != null){
		configSequence << zwave.configurationV1.configurationSet(parameterNumber: 64, size: 1, scaledConfigurationValue: settings.param64.toInteger()).format()
	}    
	if(settings.param65 != null){
		configSequence << zwave.configurationV1.configurationSet(parameterNumber: 65, size: 1, scaledConfigurationValue: settings.param65.toInteger()).format()
	}        
...
def parse(String description) {
	log.debug "Qubino Flush Heat Cool Thermostat: Parsing '${description}'"
	def result = null
...
def zwaveEvent(physicalgraph.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd){
	log.debug "Qubino Flush Heat Cool Thermostat:physicalgraph.zwave.commands.sensormultilevelv5.SensorMultilevelReport"
	def resultEvents = []
...
def zwaveEvent(physicalgraph.zwave.commands.configurationv2.ConfigurationReport cmd){
	log.debug "Qubino Flush Heat Cool Thermostat:physicalgraph.zwave.commands.configurationv2.ConfigurationReport"
	log.debug cmd
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd, physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap command){
	log.debug "Qubino Flush Heat Cool Thermostat:MC_MeterReport"
	def result = []
	switch(cmd.scale){
		case 0:
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd) {
	log.debug "Qubino Flush Heat Cool Thermostat:physicalgraph.zwave.commands.meterv3.MeterReport"
	def result = []
	switch(cmd.scale){
		case 0:
			result << createEvent(name:"kwhConsumption", value: cmd.scaledMeterValue, unit:"kWh", descriptionText:"${device.displayName} consumed ${cmd.scaledMeterValue} kWh")
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelEndPointReport cmd){
	log.debug "Qubino Flush Heat Cool Thermostat:physicalgraph.zwave.commands.multichannelv3.MultiChannelEndPointReport"
	if(cmd.endPoints > 0){
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd){
	log.debug "Qubino Flush Heat Cool Thermostat:physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap"
...
def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicReport cmd){
	log.debug "Qubino Flush Heat Cool Thermostat:physicalgraph.zwave.commands.basicv1.BasicReport"
	log.debug cmd
}
/**
...
def zwaveEvent(physicalgraph.zwave.commands.thermostatsetpointv2.ThermostatSetpointReport cmd){
	log.debug "Qubino Flush Heat Cool Thermostat:physicalgraph.zwave.commands.thermostatsetpointv2.ThermostatSetpointReport"
	def result = []
		...
		case 0:
			switch(cmd.setpointType){
				...
				case 3:
					log.debug device.currentValue("thermostatMode")
					log.debug device.currentValue("thermostatSetpoint")
					log.debug device.currentValue("autoSetpoint")
					result << createEvent(name:"thermostatSetpoint", value: convertedSp, descriptionText:"${device.displayName} setpoint at ${convertedSp}")
					result << createEvent(name:"autoSetpoint", value: convertedSp, descriptionText:"${device.displayName} auto setpoint at ${convertedSp}")
					break;
...
def zwaveEvent(physicalgraph.zwave.commands.thermostatmodev2.ThermostatModeReport cmd){
	log.debug "Qubino Flush Heat Cool Thermostat:physicalgraph.zwave.commands.thermostatmodev2.ThermostatModeReport"
	def result = []
    ...
    def autoSp = device.currentValue("autoSetpoint")
	switch(cmd.mode){
		case 0:
		...
		case 2:
			...
			break;
		case 3:
			log.debug "Auto Mode Report"
			result << createEvent(name:"thermostatOperatingState", value: "auto", descriptionText:"${device.displayName} Automatic at ${autoSp}.")
			result << createEvent(name:"thermostatMode", value: "Auto", descriptionText:"${device.displayName} in Auto mode.")
			result << createEvent(name:"modeSetting", value: "Auto mode")
			break;
	}
	return result;
}